...
def test_message_preparation_for_slack(shared_datadir):
    ...
    test_content = (shared_datadir/'test_data'/'26936_gcn_circular_dict.json').read_text()
    with patch ("scimma.apps.slack.slack_app.json.dumps") as mock_load :
        ...
        assert test_result == (shared_datadir/'expected_data'/'26936_gcn_circular_PP.txt').read_text()
...
def test_parse_slack_config(shared_datadir, capsys):
    ...
    assert json.dumps(test_result) == (shared_datadir/'expected_data'/'parsed_slack_config.txt').read_text()

    # Corrupted configuration file: missing section
    config_file_path = str(shared_datadir/'test_data'/'slack_config_dup_section.cfg')
    test_result = slack_app.parse_slack_config_file(str(config_file_path))
    ...
    assert result == "Error: Section duplication error. While reading from '' [line 15]: section 'GENERAL' already exists\n"